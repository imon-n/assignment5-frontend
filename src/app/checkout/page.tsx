    ...
    const confirmResult = await stripe.confirmPayment({
      ...
      confirmParams: {
        return_url:
          ...
          `${window.location.origin}/checkout/success?stripePaymentId=${encodeURIComponent(
            stripePaymentId
...
export default function CheckoutPage() {
  ...
  const createPaymentIntent = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:5001"}/api/v1/payments/stripe/checkout`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
